...
****
##Введение
С помощью этого руководства вы сможете попрактиковаться в создании небольшой игры. Для этого будем использовать программу виртуальной среды **Microsoft Visual Studio**, так как это самая распространенная среда для разработки и бесплатная версия полностью удовлетворяет нашим требованиям. Писать код будем на языке **С#** так как это современный объектно-ориентированный и типобезопасный язык программирования, для которого, так же, написано множество библиотек, которые можно использовать. В рамках этого руководства мы будем использовать простую библиотеку **"Small Basic"**, в которой есть всё, что нам нужно для нашей игры . Это простая библиотека с визуалом, в которой есть всё, что нам нужно для нашей игры. Следуя этому руководству шаг за шагом, в конце мы получим не только работающую, но и реагирующую на наши действия программу. **Давайте начнем!**   
****
...
## Подготовка проекта
1. Если проект создался путсым, в нем необходимо прописать:
>using System;
...
Для дальнейшей работы и удобства подключим библиотеку **"Small Basic"**. Это простая библиотека с визуалом, в которой есть всё, что нам нужно для нашей игры.
